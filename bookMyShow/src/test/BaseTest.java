...
import org.bms.controllers.*;
import org.bms.providers.InMemorySeatLockProvider;
import org.bms.providers.SeatLockProvider;
import org.bms.services.*;
...
public class BaseTest {
    ...
    public void setupController(int lockTimeOut, int allowedRetries){
        final SeatLockProvider inMemorySeatLockProvider = new InMemorySeatLockProvider(lockTimeOut);
        final BookingService bookingService = new BookingService(inMemorySeatLockProvider);
        ...
        final SeatAvailabilityService seatAvailabilityService =
                new SeatAvailabilityService(bookingService,inMemorySeatLockProvider);
        final PaymentsService paymentsService =  new PaymentsService(allowedRetries,inMemorySeatLockProvider);
        this.bookingController = new BookingController(showService,bookingService,theatreService);
        this.showController =  new ShowController(seatAvailabilityService,showService,theatreService,movieService);
        this.theatreController = new TheatreController(theatreService);
        this.movieController = new MovieController(movieService);
        this.paymentsController = new PaymentsController(paymentsService,bookingService);
    }
